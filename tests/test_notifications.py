...
from src.application.scheduler import check_and_send_reminders
...
@pytest.mark.asyncio
async def test_scheduler_threshold_logic():
    ...
    with Session(engine) as session:
        ...
        appt_24h = Appointment(
            ...
            datetime=datetime.now() + timedelta(hours=23),
...
@pytest.mark.asyncio
async def test_late_booking_protection():
    ...
    with Session(engine) as session:
        ...
        appt = Appointment(
            patient_id=patient.id,
            datetime=datetime.now() + timedelta(hours=10),
            service="Clínica Geral",
            status="confirmed"
        )
        session.add(appt)
...
@pytest.mark.asyncio
async def test_notification_failure_logging():
